...
rule download_rwi:
    ...
    """
    output:
        archive = "incoming_data/rwi/country_data/relative-wealth-index-april-2021.zip",
        csv = "incoming_data/rwi/country_data/relative-wealth-index-april-2021.csv",
    shell:
        """
...
rule process_rwi_points:
    input:
        csv = "incoming_data/rwi/country_data/relative-wealth-index-april-2021.csv",
    output:
        gpkg = "incoming_data/rwi/relative-wealth-index-april-2021.gpkg",
    run:
        import pandas as pd
...
rule process_rwi_grid:
    ...
    input:
        gpkg = "incoming_data/rwi/relative-wealth-index-april-2021.gpkg",
    output:
        tiff = "incoming_data/rwi/rwi.tif",
    shell:
        """
        gdal_rasterize \
            -a rwi \
            -init -999 \
            -a_nodata -999 \
            -tr 0.02197259999999801 0.02197259999999801 \
            -ot Float64 \
            -of GTiff \
            {input.gpkg} \
            {output.tiff}
